...
class GameCsvWriterTest @Autowired constructor(
    ...
    @Test
    fun `it successfully creates and uploads a file to s3`() {
        val s3Client = S3Client(awsConfiguration)
        val gameCsvWriter = GameCsvWriter(awsConfiguration, gameRepository)
        ...
        entityManager.flush()
        gameCsvWriter.execute()

        val target = s3Client.getContent()

        Assertions.assertTrue(target.last().startsWith("temp-file-${LocalDate.now()}"))
    }
}